...
our %EXPORT_TAGS = (
  'all' => [
    qw(
      performPIPseq_ensembl_CanFam3_1
      performPIPseq_gencode_hg38
      )
  ]
);
...
sub global_definition {
  my $result = merge_hash_right_precedent(global_options(), {
    ...
    pipseeker_docker_command => singularity_prefix(1) . " /data/cqs/softwares/singularity/fluent-pipseeker_1.0.0.sif",
...
sub performPIPseq_ensembl_CanFam3_1 {
  my ( $userdef, $perform ) = @_;
  my $def = merge_hash_left_precedent( $userdef, ensembl_CanFam3_1_genome() );
  my $config = performPIPseq( $def, $perform );
  return $config;
}
...
sub performPIPseq_gencode_hg38 {
  my ( $userdef, $perform ) = @_;
