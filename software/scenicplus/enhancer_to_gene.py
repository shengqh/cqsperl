...
def calculate_regions_to_genes_relationships(
    ...
    df_exp_mtx.iloc[:5, :5].to_csv('df_exp_mtx.head.csv')
    df_acc_mtx.iloc[:5, :5].to_csv('df_acc_mtx.head.csv')
    log.info(f"df_exp_mtx={df_exp_mtx.shape}")
    log.info(f"df_acc_mtx={df_acc_mtx.shape}")
    log.info(f"search_space={search_space.shape}")
    ...
    cur_genes = [gene for gene in region_to_gene_importances.keys()]
    log.info(f"number of important genes={len(cur_genes)}")

    if(len(cur_genes) == 0):
        log.error("No important genes found")
        return None
