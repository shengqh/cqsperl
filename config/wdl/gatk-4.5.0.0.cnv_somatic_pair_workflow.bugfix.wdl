...
task PlotDenoisedCopyRatios {
    ...
    command <<<
        ...
        #check if value is NA, if so, replace it with zero, otherwise, task will fail. by Quanhu Sheng
        value=`cat "~{output_dir_}/~{entity_id}.scaledDeltaMAD.txt"`
        if [[ "$value" == "NA" ]]; then
          echo 0 > "~{output_dir_}/~{entity_id}.scaledDeltaMAD.txt"
        fi
    >>>
