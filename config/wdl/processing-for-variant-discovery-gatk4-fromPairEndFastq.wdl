...
workflow PreProcessingForVariantDiscovery_GATK4 {
  ...
  String bwa_commandline = select_first([bwa_commandline_override, "bwa mem -K 100000000 -v 3 -t 16 -Y $bash_ref_fasta"]) 
  Int compression_level
  
  String? gatk_docker_override
  String gatk_docker = select_first([gatk_docker_override, "gatk:latest"])
  String? gatk_path_override
  String gatk_path = select_first([gatk_path_override, "/gatk/gatk"])

  String? gotc_docker_override
  String gotc_docker = select_first([gotc_docker_override, "gotc:latest"])
  String? gotc_path_override
  String gotc_path = select_first([gotc_path_override, "/usr/gitc/"])

  String? python_docker_override
  String python_docker = select_first([python_docker_override, "python:latest"])  
...
#  File? SortAndFixTagsInputBam
  ...
  # Pair end fastq to bam, keep the old name SamToFastqAndBwaMem to use the same json file as input
  # add sample_name parameter to be used in @RG tag
  call SamToFastqAndBwaMem {
      input:
        input_bams = flowcell_unmapped_bams,
  ...
  call SortAndFixTags {
    input:
#      input_bam = SortAndFixTagsInputBam,
      input_bam = select_first([MarkDuplicates.output_bam, SamToFastqAndBwaMem.output_bam]),
...
# Read fastq (Keep old name SamToFastqAndBwaMem to use old json file as input) and stream to BWA MEM for alignment
task SamToFastqAndBwaMem {
  ...
  command <<<
    ...
    # please note add @RG tags to work with GATK
		${bwa_path}${bwa_commandline} -R "@RG\tID:${sample_name}\tSM:${sample_name}\tLB:${sample_name}\tPL:illumina\tPU:${sample_name}" ${sep=' ' input_bams} 2> >(tee ${output_bam_basename}.bwa.stderr.log >&2) \
    | \
