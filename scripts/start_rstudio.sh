...
singularity exec -e --bind=/scratch/,/data,/scratch/cqs/shengq2/rstudio/rstudio-server:/var/run/rstudio-server,/scratch/cqs/shengq2/rstudio/tmp:/tmp/rstudio-server /scratch/cqs_share/softwares/singularity/rstudio.simg env PASSWORD='xyz' RSTUDIO_SESSION_TIMEOUT='0' rserver --auth-none=0  --auth-pam-helper-path=pam-helper --www-address=127.0.0.1 --www-port=8787

#Run following command in your windows prompt (replace shengq2 with your username)
#ssh -Nf -l shengq2 -L 8787:localhost:8787 cqs1.accre.vanderbilt.edu

#open http://localhost:8787/ in your windows web browser
